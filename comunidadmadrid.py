...
def pagina_filtros(escudos_dict):
    ...
    # Filtrar por goles (rango)
    if "Goles" in df.columns:
        min_goles = int(df["Goles"].min())
        max_goles = int(df["Goles"].max())
        rango_goles = st.sidebar.slider("Selecciona un rango de goles", min_goles, max_goles, (min_goles, max_goles))
    else:
        st.error("La columna 'Goles' no se encuentra en el archivo Excel.")
        return
    ...
    df_filtrado = df_filtrado[
        ...
        (df_filtrado["Goles"] >= rango_goles[0]) & (df_filtrado["Goles"] <= rango_goles[1])
    ]
...
def pagina_busqueda(escudos_dict):
    ...
    if "Goles" in df.columns:
        min_goles = int(df["Goles"].min())
        max_goles = int(df["Goles"].max())
        rango_goles = st.sidebar.slider("Selecciona un rango de goles", min_goles, max_goles, (min_goles, max_goles), key="busqueda_goles")
    else:
        st.error("La columna 'Goles' no se encuentra en el archivo Excel.")
        return
